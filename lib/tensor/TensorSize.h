...
public:
  ...
  size_t getElementsNum() const { return getDimensionNum() > 0 ? dimSize[0] : 0; }
  ...
  inline const std::vector<size_t>& getSize() const { return size; }
  ...
  // Setters
  void setRows(size_t rows) { assertIsGreater(getDimensionNum(), 0); size[getDimensionNum() - 1] = rows; dimSize[0] = rows * dimSize[1];}
